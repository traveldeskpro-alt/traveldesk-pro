...
-- 8. Subscription Plans (editable by platform super admins through RPCs)
...
INSERT INTO subscription_plans (id, name, monthly_price, user_limit, booking_limit, features)
VALUES
...
-- 9. Audit Logs (platform admin action history)
...
-- Active users on active/trial agencies can access agency-scoped rows.
-- This overrides the earlier helper definitions in fresh installs.
CREATE OR REPLACE FUNCTION public.get_my_agency_id()
  RETURNS uuid
  LANGUAGE sql
  STABLE
